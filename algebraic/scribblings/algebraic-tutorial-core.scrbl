...
@title[#:tag "tut:core" #:style '(quiet no-sidebar)]{The Core Calculus}
...
@subsection[#:tag "tut:core:example:numbers"]{Peano Numbers}
...
@subsection[#:tag "tut:core:example:arithmetic"]{Peano Arithmetic & Recursion}
...
@subsection[#:tag "tut:core:example:booleans"]{Boolean Logic}
...
@subsection[#:tag "tut:core:example:lists"]{Lists}
...
@list[algebraic-mod]. In the @seclink["tut:ext"]{next installment}, we'll
extend the surface syntax for a more realistic look and feel.
